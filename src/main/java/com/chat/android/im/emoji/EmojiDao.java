package com.chat.android.im.emoji;
...
public class EmojiDao {
    ...
    public static String CopySqliteFileFromRawToDatabases(String SqliteFileName) throws IOException {
        ...
        //复制的话这里需要换成自己项目的包名
        File dir = new File("data/data/" + (RLS.Companion.getInstance().getApp().getPackageName()) + "/databases");
        ...
        if (!file.exists()) {
            try {
                file.createNewFile();
                inputStream = RLS.Companion.getInstance().getApp().getClass().getClassLoader().getResourceAsStream("assets/" + SqliteFileName);
